...
class TagsClients {
    ...
    constructor(root) {
        ...
        this.#tags = Array.from(root.querySelectorAll('.tag'));
    ...
    #updatePositions() {
        ...
        for (let i = 0; i < N; i++) {
            ...
            this.#tags[i].style.transform = `
                translateX(calc(${translateX}px + 50%))
    ...
    #onMouseMove(e) {
        ...
        // Update tag positions on mouse move
        this.#updatePositions();
    }
...
function main() {
    document.addEventListener('DOMContentLoaded', () => {
        const root = document.querySelector('.tags-clients');
        const clients = new TagsClients(root);
    ...
    tags.forEach(tag => {
        ...
        tag.addEventListener('mouseleave', () => {
            tag.style.opacity = tag.dataset.originalOpacity;
        });

        // Store original opacity value
        tag.dataset.originalOpacity = tag.style.opacity;
    });
}
